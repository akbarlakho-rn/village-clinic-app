...
export async function restockMedicine(
  ...
  await db.withTransactionAsync(async () => {
    // 1. Update stock and optionally update purchase_price
    if (purchasePrice && purchasePrice > 0) {
      await db.runAsync(
        `UPDATE medicines 
         SET stock = stock + ?, purchase_price = ?, updated_at = datetime('now', 'localtime') 
         WHERE id = ?;`,
        [addedQuantity, purchasePrice, medicineId]
      );
    } else {
      await db.runAsync(
        `UPDATE medicines 
    ...
    await db.runAsync(
      `INSERT INTO stock_transactions (medicine_id, type, quantity, purchase_price, notes)
       VALUES (?, 'purchase', ?, ?, ?);`,
      [medicineId, addedQuantity, purchasePrice || null, notes?.trim() || 'Restocked']
    );
  });
}
// src/database/queries/medicines.ts
